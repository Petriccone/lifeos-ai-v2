  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 401) {
        ...
        setError(`Assistant unavailable (${err.status}).`);
      } else {
        setError("Assistant unavailable. Check your connection.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="glass-panel flex-1 rounded-3xl border border-white/10 flex flex-col overflow-hidden shadow-2xl relative bg-black/40">
        ...
        <header className="px-6 py-5 border-b border-white/5 flex items-center justify-between bg-white/[0.02] relative z-20 backdrop-blur-md">
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors text-gray-400"
          >
        ...
        {error && (
          <div className="mx-4 md:mx-6 mt-4 border border-red-500/30 bg-red-500/10 rounded-2xl p-3 flex items-start gap-3">
            <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
            <div className="flex-1 text-xs text-red-200">{error}</div>
            <button
        ...
        <div
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 relative z-0 scrollbar-hide"
        >
